...
var resizePizzas = function(size) {
  ...
  // This is now based on a percent width value returned from sizeSwitcher function
  ...
  // Set newWidth percentage and call changePizzaSizes to set all pizzas to new size
...
//  }
  ...
  scrollCalc = document.body.scrollTop / 1250;
  pizzaPhases = [];

  // There are only 5 phases of movement so calculate all 5 and put in array for use
  for (var i = 0; i < 5; i++) {
    pizzaPhases.push(Math.sin(scrollCalc + (i % 5)));
  }

  for (var i = 0; i < movers.length; i++) {
  //  var phase = Math.sin((document.body.scrollTop / 1250) + (i % 5));
  //  movers[i].style.left = movers[i].basicLeft + 100 * phase + 'px';
  //  Use pizzaPhases array to calculate position
    movers[i].style.left = movers[i].basicLeft + 100 * pizzaPhases[i % 5] + 'px';
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < 50; i++) {  //changed from 200
    ...
    //Changed from querySelector to getElementById to improve performance
    //document.querySelector("#movingPizzas1").appendChild(elem);
    document.getElementById("movingPizzas1").appendChild(elem);
  }
  // Add global variable for all .mover items and use getElementsByClassName
